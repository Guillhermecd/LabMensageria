...
## Camada de honestidade (Etapa 4)

- **Nunca uma rodada só.** Decisão, suíte, sweep e faixa de resultados rodam 25 sementes (`DEFAULT_ROUNDS`); os números mostrados são mediana, p95 e pior caso. A semente-mestre fica visível e há um campo para digitá-la e repetir exatamente as mesmas rodadas. O "Resultado instantâneo" continua sendo uma rodada única e vem rotulado assim, com a sua semente. Não há Web Worker: as análises rodam no servidor, nunca na thread da UI.
- **Empate por dispersão.** Dois brokers empatam quando a diferença entre as medianas dos critérios técnicos (estabilidade, latência, perda) é menor que a dispersão entre rodadas (metade da faixa p10–p90 do mais disperso, com piso de 1 ponto), em qualquer posição do ranking (`DecisionService.tiedBrokers`, campo `tiedWith`). Custo e operação não têm ruído de simulação: só desempatam. O limiar antigo (5 pontos + 90% de vitórias, aplicado só a líder e vice) foi removido.
- **Modo saturado.** Quando `carga > capacidade` (`BrokerDecision.saturation`), p50, p99 e backlog final saem do painel, porque crescem com a duração da simulação. No lugar: déficit em msg/s, tempo até o teto do broker (medido, ou extrapolado quando a simulação não chega lá), modo de falha (`DROP`, `PRODUCER_BLOCKED`, `INFLIGHT_EXHAUSTED`, `UNBOUNDED_BACKLOG`), custo acumulado da corrida e a recomendação acionável (`SaturationAdvisor`).
- **Delta modelo × simulação.** `modelErrorPct` aparece como métrica (também em ×); acima de 25% (`MODEL_ERROR_TOLERANCE_PCT`) `modelReliable` fica falso, a linha ganha "não confiável" e um alerta avisa que as estimativas do modelo não servem para decidir.
- **Consumidor a 100%.** O antd pinta a barra de verde com um check quando `percent` chega a 100, mesmo com `strokeColor` vermelho. `ConsumerUtilization` passa `status="exception"` e um ícone de alerta nesse caso.
